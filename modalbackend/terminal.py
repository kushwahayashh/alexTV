...
def add_terminal_routes(api_app, touch=None) -> None:
    ...
    # xterm.js assets load from a CDN (see terminal.html), so nothing to mount.
    ...
    async def terminal_ws(websocket: WebSocket):
        ...
        # Periodically touch activity while the WebSocket is alive
        # so the idle timeout doesn't kill the server during active terminal use.
        async def keep_alive():
            try:
                ...
                logger.exception("Activity tracker touch failed in terminal keep_alive")
