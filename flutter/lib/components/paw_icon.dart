...
/// Tabler "paw" glyph (filled). Rating marker on the hero/details facts row.
/// Ported 1:1 from the React PawIcon component — same path data, sized to the
/// surrounding text and tinted to match the fact color (like SVG `currentColor`).
class PawIcon extends StatelessWidget {
  /// Edge length in logical px. React uses 1.05em against a ~16px fact, so the
  /// default of 16.8 keeps the glyph aligned with the rating number beside it.
  final double size;
  final Color color;

  const PawIcon({super.key, this.size = 16.8, required this.color});
